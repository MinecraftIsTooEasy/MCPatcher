...
import mitemod.mcpatcher.PotionHelperMCP;
import mitemod.mcpatcher.api.IMapColor;
import mitemod.mcpatcher.api.IPotion;
import mitemod.mcpatcher.mixin.MapColorMixin;
    ...
    static void reloadMapColors(PropertiesFile properties) {
        for (int i = 0; i < MapColor.mapColorArray.length; i++) {
            if (MapColor.mapColorArray[i] != null) {
                int[] rgb = new int[] { ((jss.notfine.util.MapColorExpansion) MapColor.mapColorArray[i]).getOriginalColorValue() };
...
//                MapColor.mapColorArray[i].colorValue = rgb[0];
    ...
    public static void setupPotion(Potion potion) {
        ((jss.notfine.util.PotionExpansion) potion).setOrigColor(potion.getLiquidColor());
        potions.add(potion);
    }
