...
public class CustomAnimation implements Comparable<CustomAnimation> {
    ...
    private CustomAnimation(PropertiesFile properties, ResourceLocation srcName, ResourceLocation dstName, int mipmapLevel, int x, int y, int w, int h, ByteBuffer imageData, int numFrames) {
    ...
    void update() {
        ...
        TextureObject texture = TexturePackAPI.getTextureIfLoaded(dstName);
        if (texture == null) {
            return;
        }
        if (--currentDelay > 0) {
        ...
        update(texture.getGlTextureId(), 0, 0);
