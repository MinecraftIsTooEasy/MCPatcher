...
public class FancyDial {
    ...
    public static void setup(TextureAtlasSprite icon) {
        ...
        ResourceLocation resource = TexturePackAPI.newMCPatcherResourceLocation("/misc/" + name + ".properties");
    ...
    private FancyDial(TextureAtlasSprite icon, PropertiesFile properties) {
        ...
        TextureObject itemsTexture = TexturePackAPI.getTextureIfLoaded(TexturePackAPI.ITEMS_PNG);
        if (itemsTexture == null) {
            logger.severe("could not get items texture");
            return;
        }
        itemsFBO = new FBO(itemsTexture.getGlTextureId(), originX, originY, width, height);
